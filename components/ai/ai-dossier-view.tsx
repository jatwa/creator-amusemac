  ...
  return (
    <div className="space-y-12">
        ...
        <div className="relative z-10 space-y-6">
          ...
          <div className="space-y-2">
            <div className="flex flex-wrap items-baseline gap-3">
              ...
              <span className="text-base sm:text-lg font-mono text-tertiary">
                by {devOrg}
              </span>
            </div>
          ...
          {/* Metadata Specs Bar */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-border-subtle text-xs font-mono">
            <div>
              <span className="text-tertiary block">Introduced:</span>
              <span className="text-primary font-semibold">{entity.foundedOrIntroducedYear || "N/A"}</span>
            </div>
            <div>
              <span className="text-tertiary block">Headquarters:</span>
              <span className="text-primary font-semibold">{entity.headquarters || "Global / Distributed"}</span>
            </div>
            <div>
              <span className="text-tertiary block">Licensing:</span>
              <span className="text-primary font-semibold">{(entity.licensingModel || "Commercial").replace(/_/g, " ")}</span>
            </div>
            <div>
              <span className="text-tertiary block">Sources Verified:</span>
          ...
          <div className="pt-2 flex flex-wrap gap-2.5">
            {websiteUrl && (
              <a
                ...
                className="inline-flex items-center gap-1.5 rounded-xl bg-foreground text-background px-4 py-2 text-xs font-semibold hover:opacity-90 transition shadow-sm"
              >
            ...
            {officialLinks.webApp && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 rounded-xl border border-border bg-surface px-4 py-2 text-xs font-semibold text-primary hover:border-accent/40 transition"
              >
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          <div className="rounded-2xl border border-border bg-surface p-6 sm:p-8 space-y-4">
            <h2 className="text-xl font-bold text-primary flex items-center gap-2">
              <span className="text-accent">01 //</span> Overview & Cinematic Architecture
            </h2>
          ...
          {entity.cameraAndCinematographyFeatures && entity.cameraAndCinematographyFeatures.length > 0 && (
            <div className="rounded-2xl border border-border bg-surface p-6 sm:p-8 space-y-4">
              <h2 className="text-xl font-bold text-primary flex items-center gap-2">
                <span className="text-accent">02 //</span> Camera & Cinematography Controls
              </h2>
              <div className="grid gap-3 sm:grid-cols-2">
                {entity.cameraAndCinematographyFeatures.map((feat, i) => (
                  <div
                    key={i}
                    className="rounded-xl border border-border-subtle bg-surface-elevated/50 p-4 space-y-1"
                  >
                    <div className="text-xs font-mono text-accent font-medium flex items-center gap-2">
                      <span>🎥</span>
                      <span>Feature #{i + 1}</span>
                    </div>
                    <p className="text-xs text-secondary leading-relaxed">{feat}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-border bg-surface p-6 space-y-4">
            <h3 className="text-xs font-mono uppercase tracking-wider text-tertiary">
              Modalities & Output
            </h3>
            
            <div className="space-y-3">
              <div>
                <span className="text-[11px] font-mono text-tertiary block mb-1">
                  Primary Category:
                </span>
                <span className="rounded-md border border-border-subtle bg-surface-elevated px-2 py-0.5 text-xs font-mono text-secondary inline-block">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {capabilities.map((cap, i) => (
                      <span
                        key={i}
                        className="rounded-md bg-accent/10 text-accent px-2 py-0.5 text-xs font-medium"
                      >
                        {cap}
                      </span>
                    ))}
      ...
      {models.length > 0 && (
        <div className="rounded-3xl border border-border bg-surface p-6 sm:p-8 space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border-subtle pb-4">
            <div>
              <h2 className="text-xl font-bold text-primary flex items-center gap-2">
                <span className="text-accent">03 //</span> Verified Models & Products Ledger
              </h2>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {models.map((prod) => (
              <div
                key={prod.id}
                className="rounded-2xl border border-border bg-surface-elevated/40 p-5 space-y-3 hover:border-accent/40 transition"
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-mono font-bold text-accent">
                    {prod.version ? `v${prod.version}` : prod.commercialStatus}
                  </span>
                  <span className="rounded-full bg-surface border border-border px-2 py-0.5 text-[10px] font-mono text-tertiary">
                    {(prod.licenseType || prod.commercialStatus || "Commercial").replace(/_/g, " ")}
                  </span>
                </div>

                <div>
                  <h4 className="text-base font-bold text-primary">{prod.name}</h4>
                  <p className="text-xs text-secondary mt-1 leading-relaxed">
                    {prod.description}
                  </p>
                </div>

                <div className="pt-2 border-t border-border-subtle space-y-1 text-[11px] font-mono text-tertiary">
                  {(prod.maxResolution || prod.resolutionOrOutput) && (
                    <div className="flex justify-between">
                      <span>Max Resolution:</span>
                      <span className="text-primary font-medium">{prod.maxResolution || prod.resolutionOrOutput}</span>
                    </div>
                  )}
                  {prod.contextWindowOrDuration && (
                    <div className="flex justify-between">
                      <span>Duration / Context:</span>
                      <span className="text-primary font-medium">{prod.contextWindowOrDuration}</span>
                    </div>
                  )}
                  {prod.primaryUseCase && (
                    <div className="pt-1 text-xs font-sans text-secondary">
                      <span className="text-tertiary font-mono block text-[10px]">Best For:</span>
                      {prod.primaryUseCase}
                    </div>
                  )}
                </div>
      ...
      {(videos.length > 0 || tutorials.length > 0) && (
        <div className="rounded-3xl border border-border bg-surface p-6 sm:p-8 space-y-6">
          <div className="flex items-center justify-between border-b border-border-subtle pb-4">
            <div>
              <h2 className="text-xl font-bold text-primary flex items-center gap-2">
                <span className="text-accent">04 //</span> Verified Video Benchmarks & Tutorials
              </h2>
              <p className="text-xs text-tertiary mt-1">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {[...videos, ...tutorials].map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="rounded-2xl border border-border bg-surface-elevated/40 p-5 space-y-3 flex flex-col justify-between"
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="rounded-full bg-surface border border-border px-2 py-0.5 text-[10px] font-mono text-secondary">
                    ...
                    {item.editorialTakeaways && (
                      <div className="rounded-xl border border-border-subtle bg-surface/80 p-3 text-[11px] text-tertiary">
                  ...
                  <div className="pt-3 border-t border-border-subtle flex items-center justify-between text-xs">
                    <span className="font-mono text-tertiary">{item.sourcePlatform || "Web"}</span>
                    <a
                      href={item.sourceUrl}
                      target="_blank"
                      rel="noopener noreferrer"
      ...
      {articles.length > 0 && (
        <div className="rounded-3xl border border-border bg-surface p-6 sm:p-8 space-y-6">
          <div className="flex items-center justify-between border-b border-border-subtle pb-4">
            <div>
              <h2 className="text-xl font-bold text-primary flex items-center gap-2">
                <span className="text-accent">05 //</span> Editorial Deep Dives & Industry Coverage
              </h2>
              <p className="text-xs text-tertiary mt-1">
                Verified analysis and production case studies
              </p>
            </div>
            <span className="text-xs font-mono text-accent">{articles.length} Articles</span>
          </div>
            ...
            {articles.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="pt-3 border-t border-border-subtle flex items-center justify-between text-xs">
                    <span className="font-mono text-tertiary">{item.contentType.replace(/_/g, " ")}</span>
                    <a
                      href={item.sourceUrl}
                      target="_blank"
                      rel="noopener noreferrer"
      ...
      <div className="rounded-3xl border border-border bg-surface p-6 sm:p-8 space-y-8">
        <div>
          <h2 className="text-xl font-bold text-primary flex items-center gap-2">
            <span className="text-accent">06 //</span> Connected Cinema Knowledge Graph
          </h2>
          <p className="text-xs text-tertiary mt-1">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          ...
          {relatedTools.length > 0 && (
            <div className="rounded-2xl border border-border-subtle bg-surface-elevated/30 p-5 space-y-3">
              <span className="text-xs font-mono uppercase tracking-wider text-accent font-semibold block">
              ...
              <div className="space-y-2">
                {relatedTools.map((t) => (
                  <Link
                    key={t.id}
                    href={`/tools/${t.slug}`}
                    className="block rounded-xl border border-border bg-surface p-3 hover:border-accent/40 transition group"
                  >
                    <h5 className="text-xs font-bold text-primary group-hover:text-accent transition">
                      {t.name}
                    </h5>
                    <p className="text-[11px] text-tertiary line-clamp-1 mt-0.5">{t.tagline}</p>
                  </Link>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-2">
                {relatedTechniques.map((tech) => (
                  ...
                  >
                    ...
                    <p className="text-[11px] text-tertiary line-clamp-1 mt-0.5">{tech.creativePurpose}</p>
                  </Link>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-2">
                {relatedWorkflows.map((wf) => (
                  ...
                  >
                    ...
                    <p className="text-[11px] text-tertiary line-clamp-1 mt-0.5">{wf.summary}</p>
                  </Link>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-2">
                {relatedPrompts.map((p) => (
                  ...
                  >
                    ...
                    <p className="text-[11px] text-tertiary line-clamp-1 mt-0.5">{p.description}</p>
                  </Link>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-2">
                {relatedFilms.map((f) => (
                  ...
                  >
                    ...
                    <p className="text-[11px] text-tertiary line-clamp-1 mt-0.5">{f.logline}</p>
                  </Link>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-2">
                {relatedPeople.map((person) => (
                  ...
                  >
                    ...
                    <p className="text-[11px] text-tertiary line-clamp-1 mt-0.5">{person.primaryRole}</p>
                  </Link>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-2">
                {relatedResearch.map((r) => (
                  ...
                  >
                    ...
                    <p className="text-[11px] text-tertiary line-clamp-1 mt-0.5">{r.topic}</p>
                  </Link>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-2">
                {relatedJournal.map((j) => (
                  ...
                  >
                    ...
                    <p className="text-[11px] text-tertiary line-clamp-1 mt-0.5">{j.excerpt}</p>
                  </Link>
                ))}
              </div>
            </div>
      ...
      {entity.sources && entity.sources.length > 0 && (
        <div className="rounded-3xl border border-border bg-surface p-6 sm:p-8 space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border-subtle pb-4">
            <div>
              <h2 className="text-xl font-bold text-primary flex items-center gap-2">
                <span className="text-accent">07 //</span> 8-Tier Source & Citation Ledger
              </h2>
              <p className="text-xs text-tertiary mt-1">
          ...
          <div className="space-y-3">
            {entity.sources.map((src) => (
              <div
                key={src.id}
                className="rounded-2xl border border-border bg-surface-elevated/30 p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4"
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="rounded-full bg-accent/15 border border-accent/30 px-2.5 py-0.5 text-[10px] font-mono text-accent font-semibold">
                  ...
                  <h5 className="text-sm font-bold text-primary">{src.title || src.publisher}</h5>
                  <p className="text-xs text-secondary">{src.publisher}</p>
                </div>
                ...
                <div className="flex items-center gap-3 self-end sm:self-center">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="rounded-xl border border-border bg-surface px-3.5 py-1.5 text-xs font-semibold text-accent hover:border-accent/50 transition inline-flex items-center gap-1"
                  >
                    <span>Source</span>
                    <span>↗</span>
                  </a>
